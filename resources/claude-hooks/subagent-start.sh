#!/usr/bin/env bash
# heed claude-hooks/subagent-start.sh — emits subagent_start on SubagentStart (HD-15).
set -eu
...
printf '{"event":"subagent_start","ts":%s,"cli":"claude","thread_id":"%s","pid":%s,"pid_start":"%s","cwd":"%s","transcript_path":"%s"%s}\n' \
  "$ts" \
  "$(escape_json "$sid")" \
  "$ppid" \
